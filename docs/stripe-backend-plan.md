...
- **Does the client want plan switching (annual ↔ monthly) enabled in the Customer Portal at all, and if so, what should happen to an annual subscriber who switches to monthly?** The portal currently only exposes "Cancel" — switching was never configured. Worth flagging clearly when asking: Stripe's default behavior for an immediate switch credits the unused annual time and starts monthly billing right away — it does **not** automatically wait until the paid year is up unless specifically configured otherwise (e.g. deferring the change to period end). This has real revenue and customer-experience implications, so it's the client's decision whether to enable switching at all and which behavior they want if so — not something to decide on our end.
