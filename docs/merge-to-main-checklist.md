...
- [ ] **Recreate the two Products/Prices (monthly $250, annual $2,500) in the client's live-mode Stripe account.** Price IDs are account-specific — the test-mode IDs used on staging won't exist there. Set the resulting live Price IDs as `STRIPE_PRICE_ID_MONTHLY`/`STRIPE_PRICE_ID_ANNUAL` in the production Vercel project.
