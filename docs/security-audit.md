...
- **`npm audit` remaining totals** (after the `xlsx` fix, plus the dependency cleanup below): 6 moderate, 6 high, 0 critical — down from 15 originally. **Mitigated 2026-07-20:** `js-yaml` and `smol-toml` (nested three levels deep under `@vercel/node` → `@vercel/build-utils` → `@vercel/python-analysis`, used only for Vercel's Python-runtime build analysis, irrelevant to this all-JS/TS app) were pinned to patched versions via an `overrides` entry in `package.json` (`js-yaml@^4.3.0`, `smol-toml@^1.7.0`), since `npm audit fix` couldn't reach them without also touching their parent packages. Verified the app still builds and `drizzle-kit` still runs correctly after the bump.
  The remaining 12 (`ajv`, `esbuild`/`@esbuild-kit/*`, `minimatch`, `path-to-regexp`, `undici`, and their `@vercel/*`/`drizzle-kit` parents) are **not currently fixable without a downgrade**: `npm audit fix --force` would roll `@vercel/node` back from `5.8.26` to `4.0.0` and `drizzle-kit` back from `0.31.10` to `0.18.1` — both already the latest versions published to npm, so the "fix" is a regression to an old major version, not a real patch, and risks breaking the build/migration tooling. Left as-is deliberately: all of these are build/dev-time only (bundling functions at build time, running migrations from a developer machine) — none run in the shipped browser bundle or in the deployed serverless functions' request path. Recommend periodically checking `npm outdated` for `@vercel/node`/`drizzle-kit` rather than forcing a downgrade now.
