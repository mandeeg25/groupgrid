...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // Always collect a card, even when a free-month promotion code brings
      // today's total to $0. Otherwise the first renewal invoice has nothing to
      // charge and the subscription lapses (this happened on launch day).
